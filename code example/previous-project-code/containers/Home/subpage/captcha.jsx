...
import { getYzm } from '../../../fetch/home'
import  {Prompt} from  '../../../components/Layer'
...
class Captcha extends React.Component {
    ...
    render() {
        return (
            <div>
                <div className="yzm float-right" onClick={this.postYzm}>{this.state.getYzmTxt} </div>
                <ActivityIndicator animating={ this.state.yzmLoading } />
            </div>
        )
    }
    ...
    _postYzm(){
      ...
      if( document.querySelector('.inputs .yzm').innerText != '重新发送' && document.querySelector('.inputs .yzm').innerText != '获取验证码')return;//限制60s 之内不能连续点击
      ...
      result.then(data=>{
        ...
        this.setState({yzmLoading: false})
          if(data.code==="2"||data.code==="3")return Prompt.warning()
